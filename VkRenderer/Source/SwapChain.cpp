...
namespace vi
{
	...
	void SwapChain::Construct(const Info& info)
	{
		...
		const auto memImage = _info.allocator->Allocate(sizeof(Image) * imageCount);
		const auto memFrames = _info.allocator->Allocate(sizeof(Frame) * _MAX_FRAMES_IN_FLIGHT);
		const auto memImagesInFlight = _info.allocator->Allocate(sizeof(VkFence) * imageCount);
